    ...
    <div className="flex items-center gap-1.5">
      ...
      >
        {loading === "ban" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> :
          isBanned ? <CheckCircle className="h-3.5 w-3.5" /> : <Ban className="h-3.5 w-3.5" />
        }
        {isBanned ? "Aktif Et" : "Banla"}
      </button>

      {/* Admin / User rol toggle */}
      <button
        onClick={() => patch({ role: currentRole === "ADMIN" ? "USER" : "ADMIN" }, "role")}
        disabled={loading !== null}
        title={currentRole === "ADMIN" ? "Admini kaldır" : "Admin yap"}
        className={`p-1.5 rounded-lg border text-xs font-medium transition-colors flex items-center gap-1 ${
          currentRole === "ADMIN"
      ...
      >
        {loading === "role" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> :
          currentRole === "ADMIN" ? <ShieldOff className="h-3.5 w-3.5" /> : <ShieldCheck className="h-3.5 w-3.5" />
        }
