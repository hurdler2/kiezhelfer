  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { fetchOnline(); fetchStats(); }}
          disabled={loading}
          ...
          <div className="flex items-end gap-1 h-24">
            {stats.last30Days.map((d) => (
              <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group relative">
                <div
                  className="w-full bg-brand-500 hover:bg-brand-600 rounded-sm transition-all cursor-default"
                  style={{ height: `${Math.max((d.count / maxDay) * 80, 4)}px` }}
                />
                {/* Tooltip */}
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900">
                {lastRefresh ? lastRefresh.toLocaleTimeString("tr-TR") : "—"}
              </p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {users.length === 0 ? (
            <div className="py-16 text-center text-gray-400 text-sm">
              {loading ? "Yükleniyor..." : "Şu an aktif kullanıcı yok"}
            </div>
          ) : (
              ...
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">
                    <User className="h-3.5 w-3.5 inline mr-1" />Kullanıcı
                  </th>
                  <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">
                    <MapPin className="h-3.5 w-3.5 inline mr-1" />İlçe
                  </th>
                  <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">
                    <Globe className="h-3.5 w-3.5 inline mr-1" />IP Adresi
                  </th>
                  <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">Sayfa</th>
                  <th className="text-left px-5 py-3 text-xs text-gray-500 font-medium">
                    <Clock className="h-3.5 w-3.5 inline mr-1" />Son Görülme
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {users.map((u) => (
                  <tr key={u.userId} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 text-gray-500 text-xs max-w-[200px] truncate font-mono">
                      {u.pathname}
                    </td>
                    <td className="px-5 py-3">
